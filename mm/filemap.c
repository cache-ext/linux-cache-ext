...
static int filemap_read_folio_cachestream(struct file *file, filler_t filler,
		struct folio *folio)
{
	bool workingset = folio_test_workingset(folio);
	unsigned long pflags;
...
static int __cachestream_dio(struct file *file, struct address_space *mapping,
			     ...
			     struct folio_batch *fbatch)
{
	...
	for (size_t i = index; i < last_index; ++i) {
		...
		folio->mapping = mapping;
		folio->index = i;
		error = filemap_read_folio_cachestream(
...
retry:
	...
	if (!folio_batch_count(fbatch)) {
		...
		ret = BPF_CGROUP_RUN_PROG_CACHESTREAM(iocb, count);
		if (ret) {
			/* return filemap_create_folio(filp, mapping, */
			/* 	iocb->ki_pos >> PAGE_SHIFT, fbatch); */
			return __cachestream_dio(filp, mapping, iocb, count, fbatch);
		}
		...
		ssize_t already_read)
{
	...
	struct folio_batch fbatch;
	int i, error = 0, cachestream_flag=0;
