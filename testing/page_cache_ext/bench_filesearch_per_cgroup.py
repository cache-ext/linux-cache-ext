...
class FileSearchBenchmark(BenchmarkFramework):
    ...
    def before_benchmark(self, config):
        ...
        disable_swap()
        disable_smt()
        if (config["cgroup_name"]).startswith(DEFAULT_CACHE_EXT_CGROUP):
            recreate_cache_ext_cgroup(limit_in_bytes=config["cgroup_size"], per_cgroup=True)
            self.cache_ext_policy.start(per_cgroup=True)
        else:
