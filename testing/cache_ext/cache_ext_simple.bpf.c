...
{
	dbg_printk("cache_ext: Hi from the folio_accessed hook! :D\n");
	...
	if (inode_no != TEST_INODE_INO) {
		dbg_printk("cache_ext: Got: %llu, want %llu\n", inode_no,
			   TEST_INODE_INO);
		return;
	}
	...
	if (val == NULL) {
		...
		dbg_printk("cache_ext: Folio pointer already exists: %p\n",
			   *val);
	}
}

// SEC("struct_ops/evict_folios")
// void simple_evict_folios(struct cache_ext_eviction_ctx *eviction_ctx,
// 			 struct mem_cgroup *memcg)
void BPF_STRUCT_OPS(simple_evict_folios,
	       struct cache_ext_eviction_ctx *eviction_ctx,
	       struct mem_cgroup *memcg)
{
	dbg_printk("cache_ext: Hi from the eviction hook! :D\n");
	// Try to evict the folio pointer
	if (is_stop()) {
		return;
	}
	bpf_printk("cache_ext: Evicting folio pointer\n");
	__u64 *val = bpf_map_lookup_elem(&folio_ptr_map, &zero_key);
	if (val == NULL) {
		dbg_printk("cache_ext: Folio pointer not found\n");
		return;
	}
...
SEC(".struct_ops.link")
struct cache_ext_ops simple_ops = {
